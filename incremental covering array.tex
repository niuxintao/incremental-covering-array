...
\end{figure}
In this figure, the left side simply shows how do approach \emph{bottom-up} generate incremental covering arrays, while the right side shows the \emph{collaborative bottom-up} approach.  From this figure, we can easily learn that the bottom-up is very simple and straightforward, it only needs to build the $t$-way covering array based on the seed of $t-1$-way covering array. While for \emph{collaborative bottom-up} approach, the process is slightly more complicated. First of all, its target is to generate several incremental covering arrays. For the first one in these incremental covering arrays, it follows the \emph{bottom-up} approach, i.e., take $t-1$-way covering array as the seed to build $t$-way covering array. For the remaining incremental covering arrays, the $t$-way covering arrays needs seeding test cases from their own $t-1$-way covering arrays and some test cases from the initially generated $t+1$ covering array (the first incremental covering array). For example, in the second incremental covering array (at the middle) generated by ``collaborative bottom-up'' approach in Figure \ref{comparebottomupwithcollaborative}, we can find some part (\emph{E}) of the 3-way covering array comes from the 2-way covering array at the middle, while the other part(G) comes from the 4-way covering array at the left.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Note that the study \cite{fouche2009incremental} did not give a specific implementation of the strategy. Hence, in this paper, we implemented this strategy with the covering array generation approach--IPOG, which is the same approach used in our strategies.  By doing this, we can eliminated the influence from the covering array generation approach, and only focus on the performance among different strategies.
...
%As the number and distribution of faulty interactions are unknown, it is difficult to give a precise definition of the ``rate of fault detection'' [12,13]. But if we assume that every k-way interaction has the same probability of being faulty, and that faults arise independently, then we can give a formal definition of the rate of fault detection. Of course, both assumptions may not be realistic in practice, but in the absence of a specific fault model, these assumptions are reasonable.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{Set-ups}
For each SUT listed in Table \ref{model_of_sut_of_subjects}, we adopted four approaches i.e., \emph{bottom-up}, \emph{Collaborative bottom-up}, \emph{top-down}, and \emph{top-down hamming}, to generate an incremental covering array from 2-way to 6-way. We recorded the size of each covering array contained in this  incremental covering array, the generation time (seconds) that is consumed to generate these covering arrays. After that, we computed the RFD for each covering array.
...
%Another point that needs to note is that the approach listed in \cite{fouche2009incremental} generates three distinct incremental covering arrays for one SUT. Generating multiple covering arrays can increase their abilities to handle the random or non-determined failures.

Note that, as we mentioned before, \emph{collaborative bottom-up} generates multiple incremental covering arrays for one SUT to increase their abilities to handle the random or non-determined failures. Hence, to make a fair comparison, we recorded the average data of these covering arrays, which includes the average size, corresponding time cost and RFD, and then compared them with those of other approaches.
...
The results are listed in Fig.\ref{experiement}. There are 5 sub figures in this figure, each of which focus on reporting one specific strength covering array in the incremental covering array. For each sub-figure, there are four polygonal lines that represent the size of the generated covering array by the four approaches respectively. The x axis means the 55 subjects listed in Table \ref{model_of_sut_of_subjects}, while the y axis means the covering array size (which is normalized between 0 to 1 for visualization purposes). Note the value of "-1" in y axis means we cannot generate the specific covering array of the corresponding subject due to the limitation of memory and computing time.
...
There are several observations that can be made:
 ...
 \begin{enumerate}
 \item  Approach A performs well at the remaing at, while Approach B performs well at subjects with degree
 \item  Approach C performs well at degree ,  Approach D peforms well at degree (there are 5 in 6 better than the strategy)
 \item  Approach and Approach is similar, .
 %\item  We apply our approach on testing an industrial software and compare the performance between the traditional approach with ours.
 \item Approach and Approach varies.
 \item Approach A and B performs well at subjects with paramters number big.
 \item Approach C and D performs well at subjects with input space small (number of parameters better than ).
\end{enumerate}

In summary,  we can learn that, besides the number of subjects, \emph{Bottom-up} strategy also has a significant advantage on the extent to which it has decreased the size of test cases. In fact, \emph{Bottom-up} strategy has decreased the number of test cases ranged from 270 to 657 on average, while \emph{Top-down} strategy just decreased the number of test cases no more than 40.
...
Fig.\ref{experiement_time}  consuming time.
...
\caption{The comparison of the three strategies with respect to the RFD}
...
\begin{table}[!ht]
\caption{Compare Bottom-up strategy with Collaborative bottom-up strategy(Size)}
\label{statistic_of_subjects_size_tranditional}
\center
%\setlength{\tabcolsep}{3pt}
    \begin{tabular}{|c|c|c|c|} \hline
\multirow{2}{*}{Degree} & Bottom-up & Collaborative bottom-up & \multirow{2}{*}{Equal} \\\cline{2-3}
 & \#subjects (\#test cases) & \#subjects (\#test cases) &  \\\hline
2 & 45 (2.33) & 6 (0.83) & 4 \\
...
\begin{table}[!ht]
\caption{Compare Bottom-up strategy with Collaborative bottom-up strategy (Time)}
\label{statistic_of_subjects_time_traditional}
\center
%\setlength{\tabcolsep}{3pt}
    \begin{tabular}{|c|c|c|} \hline
    \multirow{2}{*}{Degree} & Collaborative bottom-up & Top-down \\\cline{2-3}
 & \#subjects (\#seconds) & \#subjects (\#seconds) \\\hline
2 & 54 (0.18) & 1 (0.08) \\
3 & 54 (0.82) & 1 (5.9)  \\
4 & 46 (80.19) & 2 (30.4)  \\
...
Above all, the answer to the last question is:
...
%show that strategy \emph{bottom-up} have a significant advantage over \emph{top-down} at the size of the covering arrays and time cost. It is also performs more efficient than traditional approach with respect to the time cost.
